...
public class MazeGenerator3D : MonoBehaviour
{
    ...
    [SerializeField] private Vector3Int _mazeSize = new Vector3Int(3, 3, 3); // set the size of the maze
    ...
    IEnumerator CreateGrid() // Create the grid
    {
        for (int x = 0; x < _mazeSize.x; x++)
        {
            for (int y = 0; y < _mazeSize.y; y++)
            {
                ...
                {
                    GameObject node = Instantiate(_mazeNodePrefab, new Vector3(x, z, y), Quaternion.identity, transform);
                    _grid.Add(node);
                }
    ...
    List<GameObject> GetUnvisitedNeighbors(GameObject node) // Get all info from neighbors
    {
        ...
        {
            Vector3 neighborPos = nodePosition + new Vector3(direction.x, direction.z, direction.y);
    ...
    void CreatePath(GameObject fromNode, GameObject toNode) // Create lineRenderer for the path
    {
        ...
        lineMaterial.color = Color.black; // Set color of the lineRenderer material
